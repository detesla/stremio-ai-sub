...
async function downloadSubtitle(url, season = null, episode = null) {
    try {
        ...
        if (url.endsWith('.zip')) {
            ...
            let srtEntry = null;
            if (season && episode) {
                const sCode = `S${season.toString().padStart(2, '0')}`;
                const eCode = `E${episode.toString().padStart(2, '0')}`;
                const fullCode = `${sCode}${eCode}`.toLowerCase();
                
                // Try to find the exact episode file in the zip
                srtEntry = zipEntries.find(entry => {
                    const name = entry.entryName.toLowerCase();
                    return name.endsWith('.srt') && name.includes(fullCode);
                });
            }
            
            // Fallback to the first .srt file found
            if (!srtEntry) {
                srtEntry = zipEntries.find(entry => entry.entryName.toLowerCase().endsWith('.srt'));
            }

            if (srtEntry) {
                console.log(`[SubDL] Extracted: ${srtEntry.entryName}`);
