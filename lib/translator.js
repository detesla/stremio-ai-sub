...
async function translateWithGemini(textChunks, apiKey) {
    ...
    Maintain EXACTLY ${textChunks.join('\n').split('\n').length} lines.
    ...
    for (const modelName of models) {
        try {
            console.log(`[Gemini-OpenAI] Trying ${modelName}...`);
            ...
            console.log(`[Gemini-OpenAI] Success! First line: "${translatedText[0]}"`);
            return translatedText;
        } catch (err) {
            console.warn(`[Gemini-OpenAI] ${modelName} failed:`, err.message);
            lastError = err;
        }
    }
    throw lastError;
