...
async function translateWithOpenRouter(textChunks, apiKey) {
    const openai = new OpenAI({
        apiKey: apiKey,
        baseURL: "https://openrouter.ai/api/v1",
        defaultHeaders: {
            "HTTP-Referer": "https://github.com/stremio-ai-sub",
...
let currentKeyIndex = { gemini: 0, groq: 0, puter: 0, openrouter: 0 };
...
async function translateSrt(srtContent, provider, apiKeyString) {
    ...
    if (provider === 'gemini') {
        ...
        const chunkSize = 20; // Reduced for free tier stability
        for (let i = 0; i < texts.length; i += chunkSize) {
            const chunk = texts.slice(i, i + chunkSize);
            const progress = Math.round((i / texts.length) * 100);
            console.log(`[AI Progress] OpenRouter ${progress}% - Translating lines ${i} to ${Math.min(i + chunkSize, texts.length)}...`);
            
            let success = false;
            let retries = 0;
            const maxRetries = 3;
            
            while (!success && currentKeyIndex.openrouter < apiKeys.length) {
                try {
                    // Always sleep between chunks for OpenRouter free tier
                    if (i > 0 || retries > 0) await sleep(10000); 
                    
                    const translatedChunk = await translateWithOpenRouter(chunk, apiKeys[currentKeyIndex.openrouter]);
                    if (translatedChunk && translatedChunk.length > 0) {
                        translatedTexts = translatedTexts.concat(translatedChunk);
                        success = true;
                    } else {
                    ...
                    console.error(`[OpenRouter] Attempt ${retries + 1} failed:`, errorMsg);
                    
                    if (errorMsg.includes('429') || errorMsg.includes('quota') || errorMsg.includes('overloaded')) {
                        retries++;
                        if (retries >= maxRetries) {
                            console.warn(`[OpenRouter] Max retries reached for key #${currentKeyIndex.openrouter}. Rotating...`);
                            currentKeyIndex.openrouter++;
                            retries = 0; // Reset retries for next key
                        } else {
                            console.log(`[OpenRouter] Waiting to retry...`);
                            await sleep(15000 * retries); // Backoff
                        }
                    } else {
                        console.error(`[OpenRouter] Critical error, skipping key:`, errorMsg);
                        currentKeyIndex.openrouter++;
                        retries = 0;
                    }
                }
            ...
            if (!success) {
                console.error(`[OpenRouter] ALL KEYS AND RETRIES FAILED for chunk.`);
                throw new Error(`OpenRouter translation failed after multiple attempts. The model might be too busy.`);
            }
        }
    } else {
        const chunkSize = 30; 
