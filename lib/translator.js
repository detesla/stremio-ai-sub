...
async function translateSrt(srtContent, provider, apiKeyString) {
    ...
    if (provider === 'gemini') {
        ...
        for (let i = 0; i < texts.length; i += chunkSize) {
            const chunk = texts.slice(i, i + chunkSize);
            console.log(`[AI Progress] OpenRouter - Translating lines ${i} to ${Math.min(i + chunkSize, texts.length)}...`);
            ...
            while (!success && currentKeyIndex.openrouter < apiKeys.length) {
                try {
                    ...
                    console.error(`[OpenRouter] Key #${currentKeyIndex.openrouter} failed:`, lastError);
                    
                    if (lastError.includes('429') || lastError.includes('quota') || lastError.includes('insufficient_balance')) {
                        console.warn(`[OpenRouter] Key exhausted or quota exceeded. Rotating...`);
                        currentKeyIndex.openrouter++;
                    } else {
                        // For other errors (like Auth or Model ID), try next key anyway or break if it's a configuration issue
                        currentKeyIndex.openrouter++;
                    }
                }
            }
            if (!success) {
                console.error(`[OpenRouter] ALL ATTEMPTS FAILED for chunk. Fallback to English.`);
                throw new Error(`OpenRouter translation failed: ${lastError}. Please check your API Key and Render Logs.`);
            }
        }
    } else {
        const chunkSize = 30; 
