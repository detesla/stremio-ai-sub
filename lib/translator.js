...
async function translateSrt(srtContent, provider, apiKeyString) {
    ...
    if (provider === 'gemini') {
        ...
        const chunkSize = 60; // Increased for better speed
        for (let i = 0; i < texts.length; i += chunkSize) {
            const chunk = texts.slice(i, i + chunkSize);
            ...
            while (!success && currentKeyIndex.openrouter < apiKeys.length) {
                try {
                    // Reduced sleep for faster processing
                    if (i > 0 || retries > 0) await sleep(3000); 
            ...
            if (!success) {
                ...
                throw new Error(`OpenRouter translation failed after multiple attempts.`);
            }
        }
    } else {
        const chunkSize = 30; 
